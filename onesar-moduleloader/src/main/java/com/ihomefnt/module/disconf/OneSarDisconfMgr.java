...
public class OneSarDisconfMgr implements ApplicationContextAware {

  protected static final Logger LOGGER = LoggerFactory.getLogger(DisconfMgr.class);
  protected static final OneSarDisconfMgr INSTANCE = new OneSarDisconfMgr();
  // 本实例不能初始化两次
  ...
  private OneSarDisconfMgr() {

  }

  public static OneSarDisconfMgr getInstance() {
    return INSTANCE;
  }
  ...
  protected synchronized void firstScan(List<String> scanPackageList) {
    ...
    try {
      ...
      // 扫描器
      scanMgr = OneSarScanFactory.getScanMgr(registry);
